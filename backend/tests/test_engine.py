...
def test_engine_valid_commodity_name_does_not_review_deferred_rules():
    result = evaluate(
        make_input(
            ...
            declarations={
                "manufacturer_or_packer_details": Declaration(value="ABC Foods Pvt Ltd, Mumbai, Maharashtra", confidence=0.96, source_regions=["R02"]),
                "commodity_name": Declaration(value="Potato Chips", confidence=0.96, source_regions=["R01"]),
                "consumer_care": Declaration(value={"name": "ABC Foods Consumer Care", "address": "Mumbai, Maharashtra", "telephone": "18001234567", "email": "care@abcfoods.example"}, confidence=0.96, source_regions=["R04"]),
                "net_quantity": Declaration(value=1, unit="kg", confidence=0.96, source_regions=["R03"]),
                "mrp": MoneyDeclaration(value=120, currency="INR", confidence=0.96, source_regions=["R05"]),
                "manufacture_month_year": Declaration(value="09/2026", confidence=0.96, source_regions=["R06"]),
            },
    ...
    assert all(next(item for item in result.results if item.rule_id == rule_id).status == ComplianceStatus.PASS for rule_id in expected_pass_rules)
...
def test_engine_valid_mrp_presence_passes():
    result = evaluate(make_input(declarations={"mrp": Declaration(value=120, confidence=0.96, source_regions=["R05"])}))
...
def test_engine_valid_mrp_format_passes():
    result = evaluate(make_input(text_blocks=[{"id": "R05", "text": "MRP ₹120 (inclusive of all taxes)"}], declarations={"mrp": MoneyDeclaration(value=120, currency="INR", confidence=0.96, source_regions=["R05"])}))
...
def test_engine_generic_over_25kg_is_not_applicable():
    result = evaluate(make_input(context_overrides={"package_quantity": 30, "package_quantity_unit": "kg"}))
    assert result.overall_status == OverallStatus.NOT_APPLICABLE


def test_engine_special_category_40kg_is_still_applicable():
    result = evaluate(make_input(context_overrides={"commodity_category": "agricultural_farm_produce", "package_quantity": 40, "package_quantity_unit": "kg"}))
    assert result.overall_status == OverallStatus.NON_COMPLIANT
...
def test_m1_style_quantity_json_preserves_unit_and_state():
    ...
    m1_payload = {"value": 500, "unit": "g", "confidence": 0.94, "source_regions": ["R03"], "state": "FOUND"}
    inspection = M2Input.model_validate({
        "inspection_id": "M1-CONTRACT-001", "image_id": "IMG001", "quality_status": "ACCEPTED", "quality_score": 0.94,
        "text_blocks": [{"id": "R03", "text": "Net Qty 500 g", "confidence": 0.94, "bbox": [100, 200, 300, 240]}],
        "declarations": {"net_quantity": m1_payload},
        "context": {"package_type": "retail", "commodity_category": "snacks", "commodity_measure_type": "mass", "package_quantity": 0.5, "package_quantity_unit": "kg"},
    })
...
def test_rejected_m1_quality_stops_compliance_evaluation():
    result = evaluate(make_input(quality_status="REJECTED", quality_score=0.22, declarations={"commodity_name": Declaration(value="Potato Chips", confidence=0.96, source_regions=["R01"])}))
    assert result.overall_status == OverallStatus.REVIEW_REQUIRED
...
def test_date_002_only_runs_when_best_before_is_applicable():
    declarations = {"best_before_use_by": Declaration(value="12 months", confidence=0.96, source_regions=["R07"])}
    not_applicable = evaluate(make_input(declarations=declarations, context_overrides={"best_before_use_by_applicable": False}))
    assert not any(item.rule_id == "DATE_002" for item in not_applicable.results)
    applicable = evaluate(make_input(declarations=declarations, context_overrides={"best_before_use_by_applicable": True}))
...
def test_imported_and_ecommerce_rules_route_only_when_context_applies():
    ...
    ecommerce_imported = evaluate(make_input(context_overrides={"is_ecommerce": True, "is_imported": True}))
...
def test_special_conditional_rules_route_when_context_applies():
    result = evaluate(make_input(context_overrides={"dimensions_applicable": True, "is_genetically_modified_food": True, "veg_nonveg_dot_applicable": True, "has_sticker_or_label": True, "unit_sale_price_applicable": True}))
    result_ids = {item.rule_id for item in result.results}
